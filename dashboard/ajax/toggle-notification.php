...
try {
    ...
    $query = "UPDATE saved_searches SET notification_enabled = ? WHERE id = ? AND user_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("iii", $status, $search_id, $user_id);
    
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            $message = $status ? 'Bildirimler açıldı' : 'Bildirimler kapatıldı';
