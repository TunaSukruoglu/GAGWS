    ...
    try {
        ...
        if (isset($_POST['action']) && $_POST['action'] === 'update_main_image') {
            ...
            if ($property_id > 0 && is_array($updated_images) && !empty($updated_images)) {
                ...
                $main_image = $updated_images[0]; // First image is main
                ...
                $stmt->bind_param("ssssii", $images_json, $main_image, $images_json, $main_image, $property_id, $user_id);
        ...
        // Cloudflare Images Processing - DÜZELTME: $_FILES kullan
        $cloudflareResult = processPropertyImages($_FILES['property_images'] ?? [], $edit_id ?? null, $edit_mode);
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="existing-photos-grid">
                                        <?php foreach ($existing_images as $index => $image): ?>
                                            <?php 
                                            // Generate Cloudflare URL - public formatında göster
                                            if (preg_match('/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i', $image)) {
                                                // Sadece ID ise, public URL oluştur
                                                $display_url = "https://imagedelivery.net/" . CLOUDFLARE_ACCOUNT_ID . "/" . $image . "/public";
                                            } elseif (strpos($image, '/thumbnail') !== false) {
                                                // Thumbnail URL ise public'e çevir
                                                $display_url = str_replace('/thumbnail', '/public', $image);
                                            } else {
                                                // Zaten public URL ise aynen kullan
                                                $display_url = $image;
                                            }
                                            ?>
                                            <div class="existing-photo-item cloudflare-photo" data-image="<?= htmlspecialchars($image) ?>" data-index="<?= $index ?>">
                                                <img src="<?= $display_url ?>" 
    ...
    <script>
        ...
        function makeMainPhoto(photoIndex) {
            ...
            if (confirm('Bu Cloudflare fotoğrafını ana fotoğraf yapmak istediğinize emin misiniz?')) {
                ...
                showAlert('Ana fotoğraf başarıyla değiştirildi!', 'success');
        ...
        function saveMainImageChange() {
            ...
            formData.append('property_id', <?php echo $edit_id ?? 0; ?>);
            ...
            .then(data => {
                ...
                if (data.includes('success')) {
                    showAlert('Ana resim veritabanında güncellendi!', 'success');
                } else {
                    showAlert('Ana resim güncellenemedi, sayfa yenilendiğinde eski sıra görünebilir.', 'warning');
                }
            })
            .catch(error => {
                console.error('Main image update error:', error);
                showAlert('Ana resim güncelleme hatası!', 'error');
            });
        }
        ...
        function updateExistingPhotosDisplay() {
            ...
            existingPhotosData.forEach((image, index) => {
                ...
                // Generate Cloudflare thumbnail URL for display - UPDATED ACCOUNT ID
    ...
    function addPhotoToGrid(file, index, isCloudflare = false) {
        ...
        photoItem.innerHTML = `
            ...
            <div class="photo-controls">
                ...
                <button type="button" class="btn-make-main" onclick="setAsMain(this)" title="Ana resim yap" ${index === 0 ? 'style="display:none"' : ''}>
                    <i class="fas fa-star"></i>
                </button>
            </div>
        ...
        updatePhotoCounter();
                        ...
                        ${index > 0 ? `<button type="button" class="photo-action-btn main" onclick="makeMainPhoto(${index})">
                            <i class="fas fa-star"></i>
                        </button>` : ''}
                    </div>
                </div>
                ${index === 0 ? '<div class="main-photo-badge"><i class="fas fa-star"></i> ANA</div>' : ''}
                <div class="cloudflare-badge">☁️ CF</div>
            `;
        };
