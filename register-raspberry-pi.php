...
// SMTP Email gönderme fonksiyonu (Raspberry Pi için)
function sendSMTPEmail($to, $subject, $message, $from_name = 'Gökhan Aydınlı Gayrimenkul', $from_email = 'no-reply@sunucu.dev') {
    // MXRouting mail server ayarları
    $smtp_host = 'blizzard.mxrouting.net'; // MXRouting SMTP Server
    ...
    try {
        ...
        // İlk yanıtı oku (220 beklenir)
        $response = fgets($socket, 515);
        if (substr($response, 0, 3) !== '220') {
            throw new Exception("SMTP sunucu yanıtı beklenmeyen: $response");
        }
        ...
        foreach ($commands as $command) {
            ...
            do {
                ...
                $full_response .= $line;
                if (empty($response)) {
                    $response = $line; // İlk satırı ana yanıt olarak al
                }
                // Çok satırlı yanıt kontrolü (250-xxx devam ediyor, 250 xxx bitiyor)
                $continue = (strlen($line) >= 4 && $line[3] === '-');
            } while ($continue && !feof($socket));
            
            // Yanıt kodlarını kontrol et
            $code = substr($response, 0, 3);
            if ($command === 'EHLO' && $code !== '250') {
                throw new Exception("EHLO komutu başarısız: $response");
            }
            if (strpos($command, 'MAIL FROM') === 0 && $code !== '250') {
                throw new Exception("MAIL FROM komutu başarısız: $response");
            }
            if (strpos($command, 'RCPT TO') === 0 && $code !== '250') {
                throw new Exception("RCPT TO komutu başarısız: $response");
            }
            if ($command === 'DATA' && $code !== '354') {
                throw new Exception("DATA komutu başarısız: $response");
            }
        }
        
        // Email headerları ve içeriği
        ...
        // Email içeriğini gönder
        fputs($socket, $email_data);
        $response = fgets($socket, 515);
        
        if (substr($response, 0, 3) !== '250') {
            throw new Exception("Email gönderimi başarısız: $response");
        }
...
<body>
    <div class='container'>
        ...
        <div style='text-align: center; color: #666; border-top: 1px solid #eee; padding-top: 20px; margin-top: 30px; font-size: 12px;'>
            <p>Bu email Raspberry Pi mail server üzerinden gönderilmiştir.</p>
            ...
            try {
                ...
                $email_sent = true;
                $email_method = 'Raspberry Pi SMTP';
                logEmail($email, $subject, 'SENT', $email_method);
            } catch (Exception $e) {
                logEmail($email, $subject, 'FAILED', 'Raspberry Pi SMTP', $e->getMessage());
                ...
                try {
                    ...
                    if (mail($email, $subject, $message, $headers)) {
                        $email_sent = true;
                        $email_method = 'PHP mail() (fallback)';
                        logEmail($email, $subject, 'SENT', $email_method);
                    } else {
                        logEmail($email, $subject, 'FAILED', 'PHP mail() (fallback)', 'mail() fonksiyonu false döndü');
                    }
                } catch (Exception $e2) {
                    logEmail($email, $subject, 'ERROR', 'PHP mail() (fallback)', $e2->getMessage());
                }
            }
            
            if ($email_sent) {
                $_SESSION['register_success'] = "✅ Kayıt başarılı! <strong>" . htmlspecialchars($email) . "</strong> adresine doğrulama email'i gönderildi. (Method: $email_method)";
            } else {
                $_SESSION['register_success'] = "⚠️ Kayıt başarılı ancak email gönderilemedi. <a href='raspberry-pi-email-test.php'>Raspberry Pi Email Test</a> sayfasından kontrol edin.";
            }
