<?php
// Akıllı resim servisi - Cloudflare Images ve local boyutlara göre yönlendirme
...
try {
    ...
    // Cloudflare yüklenemezse sadece local kullan
    error_log("Cloudflare config yüklenemedi: " . $e->getMessage());
}
...
// CLOUDFLARE IMAGES KONTROLÜ
if (class_exists('CloudflareImagesConfig') && CloudflareImagesConfig::isEnabled()) {
    // Cloudflare ID kontrolü (UUID format)
    if (preg_match('/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i', $requested_image)) {
        try {
            $cloudflare = new MultiDomainCloudflareImages();
            $cloudflare_url = $cloudflare->getImageUrl($requested_image, $width, $height);
            
            if ($debug) {
                echo "DEBUG: Cloudflare URL: $cloudflare_url<br>";
                echo "DEBUG: Redirecting to Cloudflare...";
            } else {
                // Cloudflare'a yönlendir
                header("Location: $cloudflare_url");
                exit();
            }
        } catch (Exception $e) {
            if ($debug) {
                echo "DEBUG: Cloudflare error: " . $e->getMessage() . "<br>";
            }
            error_log("Cloudflare image error: " . $e->getMessage());
            // Local'e fallback yap
        }
    } else if ($debug) {
        echo "DEBUG: Not a Cloudflare ID format, checking local files<br>";
    }
}
